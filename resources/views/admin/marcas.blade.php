...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Marcas
            <small>Gestionar Marcas de Artículos</small>
        </h1>
        <ol class="breadcrumb">
                ...
                <div class="box">
                    <div class="box-body">
                        <div class="box-header">
                            <h3 class="box-title">Importar o Exportar Listado de Marcas</h3>
                        </div>
                        <form method="POST" action="{{ route('import_m') }}" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div style="margin:10px 0px 10px 0px;" class="row">
                                <div class="col-sm-4">
                                    ...
                                    <a class="btn btn-warning" href="{{ route('export_m') }}">Exportar Datos</a>
                                </div>
                            ...
                            <tbody>
                                @foreach($marcas as $marca)
                                        <tr>
                                            <td></td>
                                            <td>{{ $marca->codigo }}</td>
                                            <td id="{{ $marca->codigo }}" class="edit_marca">{{ $marca->marca }}</td>
                                            <td id="{{ $marca->codigo }}" class="">
                                                <a data-id="{{ $marca->codigo }}" data-toggle="modal" data-target="#img-modal" class="fa fa-cloud-upload fa-2x imagen_codigo"></a>
                                            </td>
                                            @isset($marca->ruta_imagen)
                                                <input id="{{ $marca->codigo }}" class="show_img" type="hidden" value="{{ $marca->ruta_imagen }}">
                                            @endisset
                                            
                                            @empty($marca->ruta_imagen)
                                                <input id="{{ $marca->codigo }}" class="show_img" type="hidden" value="/storage/no_product.png">
                                            @endempty
                                        </tr>
                                @endforeach
                            </tbody>
    ...
    <div class="modal fade" style="padding-left: 17px;" id="img-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="box">
            <div class="box-body">
                <div class="box-header">
                    <h3 class="box-title">Editar Imagen de la Marca</h3>
                </div>
                <form method="POST" action="{{ route('imgs_m') }}" enctype="multipart/form-data">
                    @csrf
                    {{method_field ('put')}}
                    <input id="code_img" name="code_img" type="hidden" value="">
                    <div class="file-loading">
                        <input id="input-img" name="inputimg[]" type="file" data-browse-on-zone-click="true">
                    </div>
                    <div style="margin:10px 0px 10px 0px;" class="row">
                        <div class="col-sm-4">
...
@push('scripts')
    ...
    //Editar marca al vuelo
    $('#articulos_table').editable({
      container: 'body',
      selector: 'td.edit_marca',
      url: function(params) {
          var codigo_actual = $(this).attr('id');
          $.get("marcas/update_marca?marca="+params.value+'&code='+codigo_actual).done(function(){
